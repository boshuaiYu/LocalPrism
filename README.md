...
The stable updater endpoint is `releases/latest/download/latest.json`. GitHub only points that URL at the newest non-prerelease, so a beta must stay a GitHub prerelease (or a tag such as `v1.0.8beta3`) and must not be published as Latest. With **Join prerelease / Beta** off, update checks use that stable endpoint only. Turn the switch on in the bottom bar to also look at the GitHub releases list. A newer beta is offered from `releases/download/<tag>/latest.json` and shows as flashing text next to the version; it is not downloaded until you click that text. A newer stable build still downloads in the background. The WiX-safe version `1.0.8-2` is the same post-release build as `1.0.8beta2`, and that class is newer than the plain tag `1.0.8` while Beta stays on, so Latest `1.0.8` does not replace an installed `1.0.8-2` until Beta is turned off. Turning Beta off offers that stable release so the install can leave the beta channel. `1.0.8beta3` is newer than `1.0.8-2`. A word prerelease such as `1.0.8-beta.2` stays older than plain `1.0.8`.
