                ...
                if self.config.semi_static_mode {
                    ...
                    for libbase in &state.libs.libs {
                        let do_static = match libbase.as_ref() {
                            // libc family is always shared.
                            "c" | "m" | "dl" | "pthread" | "rt" => false,
                            // GLib (and the libraries it is built against) MUST
                            // stay shared. GTK, GIO, GDK, and WebKitGTK are
                            // already linked to the system libglib. Semi-static
                            // mode otherwise absorbs libglib-2.0.a because
                            // HarfBuzz/Fontconfig depend on it, and Rust then
                            // exports those g_* symbols from the executable.
                            // The dynamic linker lets them interpose on the
                            // system GLib, so GTK and GIO run a mix of two
                            // GLib ABIs. On Debian 13 (GLib 2.84) that is a
                            // null-pointer SIGSEGV inside g_bus_get_sync during
                            // g_application_register; on Ubuntu 24.04 (GLib
                            // 2.80 vs the Ubuntu 22.04 static copy) it is
                            // `free(): invalid pointer` from g_string_free.
                            "glib-2.0" | "gobject-2.0" | "gio-2.0" | "gmodule-2.0"
                            | "gthread-2.0" | "ffi" | "pcre" | "pcre2-8" | "intl" => false,
                            _ => {
